  ...
  for (idx = 0; idx < afl->queued_paths; idx++) {
    ...
    switch (res) {
      ...
      case FSRV_RUN_OK:
        ...
        /*********************************
         * Capability Guided Fuzzing HACK!
         *********************************/
        /* we will take all given seed as initial seeds, that is, we will always
         * believe the user input, even though they won't crash */
        if (afl->crash_mode) {
          SAYF(cGRA "Test case '%s' does *NOT* crash, "
               "len = %u, map size = %u, exec speed = %llu us\n" cRST,
               fn, q->len, q->bitmap_size, q->exec_us);
      ...
      case FSRV_RUN_TMOUT:
        ...
        if (afl->timeout_given && !afl->afl_env.afl_exit_on_seed_issues) {
          ...
          if (!q->was_fuzzed) {
            ...
            q->was_fuzzed = 1;
            update_union_fuzzed(q->union_hash);
...
#undef MSG_FORK_ON_APPLE
  ...
  // TODO: remove all entries from the queue that have a duplicate capability
...
static void handle_existing_out_dir(afl_state_t *afl) {
  ...
  fn = alloc_printf("%s/seeds", afl->out_dir);
  if (delete_files(fn, NULL)) { goto dir_cleanup_failed; }
  if (afl->in_place_resume && rmdir(fn)) {
  ...
  fn = alloc_printf("%s/unique-states", afl->out_dir);
...
void setup_dirs_fds(afl_state_t *afl) {
  ...
  tmp = alloc_printf("%s/seeds", afl->out_dir);
  if (mkdir(tmp, 0700)) { PFATAL("Unable to create '%s'", tmp); }
  ck_free(tmp);

  tmp = alloc_printf("%s/unique-states", afl->out_dir);
  if (mkdir(tmp, 0700)) { PFATAL("Unable to create '%s'", tmp); }
  ck_free(tmp);
