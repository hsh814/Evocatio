...
#define LATEST_SEED_NUM 20       /* Used to decide the most_active_union */
#define MAX_UNION_LEN 500         /* number of positions in union at most */
#define EXPLORE_TIME 600000       /* union explore time, ms */

typedef struct cap_union {

  u32 union_len;                        /* Number of bytes in union         */
  u32 union_heat;                       /* Union heat in the heatMap        */
  u32 union_hash;                       /* union hash for the test case */

  u8 is_most_active;
  u8 is_fuzzed;

  u32 mutate_pos[MAX_UNION_LEN];                      /* positions compose this union */
  u32 pos_num;                          /* number of position this union has */
...
u32 fuzzed_union[MAX_UNION_NUM];
u32 fuzzed_union_cnt;
...
struct queue_entry {
  ...
  int cur_mutated_pos[MAX_MUTATE_NUM];  /* current seed mutation position */
  int cur_mutate_pos_num;               /* how many bytes we have mutated in current? */
...
#define CRITICAL_BYTES_NUM 1024   // How many critical bytes we can have at most
...
/**** Capability Guided Fuzzing Related ****/
...
struct byte_info {
  ...
  u32 heat;                   /* How many capability this byte can introduce? */
};
...
typedef struct afl_state {
  ...
  u32 virgin_capability;               /* Capability hash of original poc */
...
void capability_state_destroy(afl_state_t *afl);
...
s32 read_constraints_from_file(afl_state_t *afl, struct byte_info byteInfo[]);
...
u8 has_new_capability(afl_state_t *afl, struct queue_entry *q, void *mem);
void scan_seed_capability(afl_state_t *afl, struct queue_entry *q, u8 *use_mem);
u8 update_union_fuzzed(u32 union_hash);
...
/* Count critical bytes number */
int count_critical_pos(struct byte_info bMap[], u32 bMap_size);
/* Parse critical bytes position */
void get_critical_pos(struct byte_info bMap[], int *result, u32 bMap_size);
...
/* Pick a random union from union set */
static inline cap_union_t rand_union() {
  u32 index;
  /* Generate a random index */
  index = rand() % all_union_num;

  return all_union[index];
}

/* Pick a random byte from union */
static inline u32 rand_byte_from_union(cap_union_t uni) {
  u32 index, union_size;

  union_size = uni.union_len;

  index = rand() % union_size;

  return uni.mutate_pos[index];

}

/* Pick a random bit from byte */
static inline u32 rand_bit_from_byte(u32 byte_addr) {
  u32 index;
  u32 rand_addr, addr_in_bits, addr_in_bits_min;

  addr_in_bits = byte_addr << 3;
  addr_in_bits_min = addr_in_bits - 8;

  index = rand() % 8;

  rand_addr = addr_in_bits - index;

  if (addr_in_bits_min <= rand_addr && rand_addr <= addr_in_bits) {
    return byte_addr + index;
  }
...
/* Generate a random position from C-D map. Only the position
 * belong to critical byte will be generated. This may
 * have slight bias. */

/* TODO: check pos to avoid invalid access! */
static inline u32 rand_from_map(int map[]) {
  int index_rnd;
  int map_len = 0;
  map_len = sizeof(map)/sizeof(int);

  /* Generate a random index */
  index_rnd = rand() % map_len;

  return map[index_rnd];
}
